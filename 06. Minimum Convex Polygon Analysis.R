# =========================================================
# 06 — GAP ANALYSIS ADAPTADA DE RODRIGUES et al. (2004)
...
# - A proteção por pontos continua disponível no script 05 e deve ser usada
...
pontos_proj <- dados_sf |> dplyr::filter(!is.na(scientificName_det), scientificName_det != "", !sf::st_is_empty(geometry)) |> sf::st_transform(crs_area)
...
calcular_meta_rodrigues <- function(area_distribuicao_km2) {
  ...
  posicao_log <- (log10(area_distribuicao_km2) - log10(limite_distribuicao_restrita_km2)) / (
    log10(limite_distribuicao_ampla_km2) - log10(limite_distribuicao_restrita_km2))
  meta_distribuicao_restrita - posicao_log * (meta_distribuicao_restrita - meta_distribuicao_ampla)
}
